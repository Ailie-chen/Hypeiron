...
DATE_SET="0104"
NUM_CORE=4
prefsL1=( "ipcp_isca2020" )
prefsL2=( "ipcp_isca2020")
# prefsL1=( "vberti" )
...
warmupInstructions=50000000
simulationInstructions=200000000
...
function compile_champsim
{
  ...
  # 执行构建命令
  ./build_champsim.sh hashed_perceptron no $1 $2 no no no no lru lru lru srrip drrip lru lru lru 4 no $DATE_SET
...
function get_ChampSimPath
{
  ...
  # - $1 $2: 预取器名称
  ##########
  champSimPath="./ChampSim/Berti/bin/${DATE_SET}hashed_perceptron-no-$1-$2-no-no-no-no-lru-lru-lru-srrip-drrip-lru-lru-lru-4core-no"
...
function generate_par_multi_core
{
  ...
  idx=0
  trace=""
  trace_lists=( "${NUM_CORE}core_heterogeneous.in" "${NUM_CORE}core_homogeneous1.in" )
    ...
    do   
        if [[ ! -z $line ]]; then
          ...
          if [ "$suite_name" == "cs" ]; then
            echo  "$4 -warmup_instructions $warmupInstructions -simulation_instructions $simulationInstructions -cloudsuite -traces $trace > $2-${3}-${5}-no---$idx.out 2>/dev/null" >> $tmp_par_path
          else
            echo  "$4 -warmup_instructions $warmupInstructions -simulation_instructions $simulationInstructions -traces $trace > $2-${3}-${5}-no---$idx.out 2>/dev/null" >> $tmp_par_path
...
main()
{
  ...
  for prefL1 in "${prefsL1[@]}"
    do
    for prefL2 in "${prefsL2[@]}"
      do
        #判断是否需要编译
        if [ "$1" == "compile" ]; then
          compile_champsim $prefL1 $prefL2
        fi

        #根据trace获取tmp_par
        # 设置ChampSim的bin路径
        champSimPath=$(get_ChampSimPath $prefL1 $prefL2)
        # 清空tmp_par文件
        
        
        generate_par_multi_core $trace_base_path "$output_base${NUM_CORE}core/" $prefL1 $champSimPath $prefL2
      done
  done
