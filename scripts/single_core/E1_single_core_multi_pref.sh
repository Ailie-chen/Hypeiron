...
function compile_champsim
{
  ...
  # 执行构建命令
  ./build_champsim.sh hashed_perceptron no $1 $2 no no no no lru lru lru srrip drrip lru lru lru 1 no $DATE_SET
...
function get_ChampSimPath
{
  ...
  # - $1 $2: 预取器名称
  ##########
  champSimPath="./ChampSim/Berti/bin/${DATE_SET}hashed_perceptron-no-$1-$2-no-no-no-no-lru-lru-lru-srrip-drrip-lru-lru-lru-1core-no"
...
function generate_par
{
  ...
  do
    ...
    outputFile="$2/-${3}-${6}-no---$traceFilebase"
    ...
    for prefL2 in "${prefsL2[@]}"
      do
        ...
        do
            ...
            generate_par $trace_file_path $res_output_path $prefL1 $champSimPath $trace $prefL2
        done
      done
  done
