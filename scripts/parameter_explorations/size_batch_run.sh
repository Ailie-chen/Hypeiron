...
main()
{
    ...
    for P1 in "${P1S[@]}"; do
        for P2 in "${P2S[@]}"; do
                pref="hyperion_hpc_${P1}_${P2}"
                # pref="no"
                destination_file="ChampSim/Berti/batch_prefetcher/${pref}.l1d_pref"
                cp "$source_file" "$destination_file"
                sed -i "s/const float L1_THRESHOLD=0.80/const float L1_THRESHOLD=${P1}/" "$destination_file"
                sed -i "s/const float L2_THRESHOLD=0.20/const float L2_THRESHOLD=${P2}/" "$destination_file"
                prefs+=("$pref")
        done
    done
