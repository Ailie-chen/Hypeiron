...
main()
{
    ...
    P1S=( 0.50 0.60 0.70 0.80 0.90 1.0)
    P2S=( 0.10 0.20 0.30 0.40 0.50 )
    prefs=()
    for P1 in "${P1S[@]}"; do
        for P2 in "${P2S[@]}"; do
                pref="hyperion_2table_${P1}_${P2}"
                # pref="no"
                destination_file="ChampSim/Berti/batch_prefetcher/${pref}.l1d_pref"
                cp "$source_file" "$destination_file"
                sed -i "s/const double L1_THRESHOLD = 0.80/const double L1_THRESHOLD = ${P1}/" "$destination_file"
                sed -i "s/const double L2_THRESHOLD = 0.20/const double L2_THRESHOLD = ${P2}/" "$destination_file"
                prefs+=("$pref")
        done
    done
    ...
    # P1S=( 0.80 )
    # P2S=( 0.0 0.10 0.20 0.30 0.40 0.50)
    # prefs=()
    # for P1 in "${P1S[@]}"; do
    #     for P2 in "${P2S[@]}"; do
    #             pref="hyperion_2table_${P1}_${P2}"
