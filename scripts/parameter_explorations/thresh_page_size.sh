...
main()
{
    ...
    source_file="ChampSim/Berti/batch_prefetcher/hyperion_hpc_2table_UTBh1_buffer9_3.l1d_pref"
    cp "ChampSim/Berti/prefetcher/hyperion_hpc_2table_UTBh1_buffer9_3.l1d_pref" "$source_file"
    P1S=( 2 4 8 16 32 64 128 256 )
    prefs=()
    for P1 in "${P1S[@]}"; do
        pref="hyperion_hpc_page_size_${P1}"
        # pref="no"
        destination_file="ChampSim/Berti/batch_prefetcher/${pref}.l1d_pref"
        cp "$source_file" "$destination_file"
        echo ${P1}
        sed -i "s/const uint64_t REGION_SIZE = 32 \* 1024/const uint64_t REGION_SIZE = ${P1} \* 1024/" "$destination_file"
        prefs+=("$pref")
    done
