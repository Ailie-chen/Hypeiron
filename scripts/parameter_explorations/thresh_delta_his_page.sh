...
main()
{
    ...
    P1S=( 16 32 64 128 256)
    prefs=()
    for P1 in "${P1S[@]}"; do
        pref="hyperion_hpc_pagestride_${P1}"
        # pref="no"
        destination_file="ChampSim/Berti/batch_prefetcher/${pref}.l1d_pref"
        cp "$source_file" "$destination_file"
        sed -i "s/const uint64_t PAGE_STRIDE_TABLE_SIZE = 64/const uint64_t PAGE_STRIDE_TABLE_SIZE = ${P1}/" "$destination_file"
        prefs+=("$pref")
    done
