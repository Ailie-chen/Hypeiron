...
main()
{
    ...
    source_file="ChampSim/Berti/batch_prefetcher/hyperion_hpc_2table_UTBh1_buffer9_2.l1d_pref"
    cp "ChampSim/Berti/prefetcher/hyperion_hpc_2table_UTBh1_buffer9_2.l1d_pref" "$source_file"
    P1S=( 2 4 6 8 10 12 )
    prefs=()
    for P1 in "${P1S[@]}"; do
        pref="hyperion_issue_num_${P1}"
        # pref="no"
        destination_file="ChampSim/Berti/batch_prefetcher/${pref}.l1d_pref"
        cp "$source_file" "$destination_file"
        echo ${P1}
        sed -i "s/const int ALL_MAX_PF = 12/const int ALL_MAX_PF = ${P1}/" "$destination_file"
        prefs+=("$pref")
    done
