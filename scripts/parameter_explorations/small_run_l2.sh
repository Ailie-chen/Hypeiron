...
function compile_champsim
{
  ...
  # 执行构建命令
  ./build_champsim.sh hashed_perceptron no no $1 no no no no lru lru lru srrip drrip lru lru lru 1 no $DATE_SET
...
function get_ChampSimPath
{
  ...
  ##替换为l2
  champSimPath="./ChampSim/Berti/bin/${DATE_SET}hashed_perceptron-no-no-$1-no-no-no-no-lru-lru-lru-srrip-drrip-lru-lru-lru-1core-no"
...
function generate_par
{
  ...
  do
    ...
    # outputFile="$2/hashed_perceptron-no-${3}pg-no-no-no-no-no-lru-lru-lru-srrip-drrip-lru-lru-lru-1core-no---$traceFilebase"
    # outputFile="$2/$traceFilebase"    
    outputFile="$2/-${3}l2-no---$traceFilebase"
    ...
    if [[ $5 == "cs" ]];then
      ...
      echo "$4 -warmup_instructions $warmupInstructions -simulation_instructions $simulationInstructions -traces $1$traceFilebase > $outputFile 2>/dev/null" >> ${tmp_par_path}
    fi
  done
}
