...
main()
{
    ...
    source_file="ChampSim/Berti/batch_prefetcher/hyperion_hpc_aDiGHB_UTBh1_buffer8_8.l1d_pref"
    cp "ChampSim/Berti/prefetcher/hyperion_hpc_2table_UTBh1_buffer8_8.l1d_pref" "$source_file"
    # PCNS=( 4 8 16 32 64 128 )
    PCNS=( 4 8 16 )
    PAGENS=( 4 8 16 32 64 128 )
    MUL=( 1 2 )
    MULS=( 1 2 3 )
    prefs=()
    for pcn in "${PCNS[@]}"; do
      for pc_page in "${MULS[@]}"; do
        for mpc in "${MUL[@]}"; do
          for mpage in "${MUL[@]}"; do
            if [ $pcn -eq 4 ]&&[ $pc_page -eq 1 ]; then
              continue
            fi
            if [ $pcn -eq 16 ]&&[ $pc_page -eq 2 ]; then
              continue
            fi
            if [ $pcn -eq 16 ]&&[ $pc_page -eq 3 ]; then
              continue
            fi
            if [ $pc_page -eq 1 ]; then
              pagen=$(($pcn/2))
            else 
              if [ $pc_page -eq 2 ]; then
                pagen=$(($pcn))
              else
                pagen=$(($pcn*2))
              fi
            fi
            pref="hyperion_2table_size_${pcn}_${mpc}_${pagen}_${mpage}"            # pref="no"
            destination_file="ChampSim/Berti/batch_prefetcher/${pref}.l1d_pref"
            cp "$source_file" "$destination_file"
            sed -i "s/const int PC_HISTORY_SIZE = 16/const int PC_HISTORY_SIZE = ${pcn}/" "$destination_file"
            sed -i "s/const int PAGE_HISTORY_SIZE = 32/const int PAGE_HISTORY_SIZE = ${pagen}/" "$destination_file"
         if [ $pcn -ge $pagen ]; then
              sed -i "s/const int GHB_SIZE = 256/const int GHB_SIZE = $((${pcn}*8))/" "$destination_file"
            else
              sed -i "s/const int GHB_SIZE = 256/const int GHB_SIZE = $((${pagen}*8))/" "$destination_file"
            fi  
            sed -i "s/const uint64_t PAGE_STRIDE_TABLE_SIZE = 64/const uint64_t PAGE_STRIDE_TABLE_SIZE = $((${pagen}*${mpage}))/" "$destination_file"
            sed -i "s/const uint64_t PC_STRIDE_TABLE_SIZE = 16/const uint64_t PC_STRIDE_TABLE_SIZE = $((${pcn}*${mpc}))/" "$destination_file"
            prefs+=("$pref")     
          done
        done
      done
