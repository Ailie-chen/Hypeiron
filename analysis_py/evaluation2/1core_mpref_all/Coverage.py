...
metrics=['L2C Coverage','LLC Coverage']
...
if __name__ == "__main__":  
    for date in dates:
        ...
        #设置子图间隔
        plt.subplots_adjust(left=0, right=1.0, bottom=0, top=1.0, wspace=0.17, hspace=0.37)
        # 设定条形图的宽度
        ...
        for trace in traces:
            ...
            for metric in metrics:       
                ...
                i = i + 1
            min_v = 1.0
            max_v = data_all[1:num_prefs,:].max().max()+0.1
            legend_patch = []
            ...
            axes[ay].set_xticklabels(['L2','LLC'], fontsize=9)
            # axes[ay].set_xlabel('Benchmarks', fontsize=9)
            print(max_v)
            if(max_v < 0.5):
                y_ticks = np.around(np.arange(0, 0.51, 0.10), decimals=2)
                y_ticks1 = np.around(np.arange(0, 0.51, 0.05), decimals=2)
                min_v = 0
                max_v = 0.5
            else:
                y_ticks = np.around(np.arange(0.5, 1.01, 0.1), decimals=2)
                y_ticks1 = np.around(np.arange(0.5, 1.01, 0.05), decimals=2)
                min_v = 0.5
                max_v = 1.0
            ...
            if(ax== 0 and ay == 0):
                axes[ay].legend(handles=legend_patch,loc='upper center', bbox_to_anchor=(1.55, 1.63), ncol=4, fontsize=9,
             # y 参数控制标题的位置
        ...
        plt.savefig(f'{figure_res}/1core_mpref_coverage.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/1core_mpref_coverage.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.close()   
