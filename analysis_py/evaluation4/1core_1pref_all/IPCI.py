...
#         # 旋转之后还需要向左平移
#         plt.xticks(left_bar_pos + bar_width*((num_prefs-1)/2+0.5), [dic_suite[item] for item in traces], rotation=0, fontsize=9)
#         # plt.yticks([-1, -0.5, 0, 0.5, 1], font=font["family"], fontsize=16)
#         plt.ylim(min_v, max_v)
#         for label in ax.get_yticklabels():
#             label.set_fontsize(8)
#         # ax.set_xlabel('Benchmarks Suites', fontsize=9)
#         ax.set_ylabel("SpeedUp", fontsize=9)
#         # ax.set_title(f'IPCI', fontsize=9)
#         ax.legend(loc='upper center', bbox_to_anchor=(0.5, 0.83*fig_size[1]), ncol=num_prefs-1,fontsize=9,
...
Benchs={"ipcp_isca2020":"IPCP",
         ...
         "vberti":"Berti",
         "vberti_ft3":"Berti_Ehance",
         "hyperion_hpc_aDiGHB_UTBh":"Hyperion",
         "bingo_dpc3":"Bingo",
         "ppf":'SPP-PPF'}
hatches = ['\\\\\\\\\\',   '', '', '', ''] #'xxxxx',
colors = [ 'white','white','grey','black']
    ...
    for date in dates:
        ...
        for trace in traces:
            ...
            for metric in metrics:       
                ...
                i = i + 1
            print(data_all[0:num_prefs,i-1])     
            min_v = 1.0
            ...
            y_ticks1=[]
            if(max_v < 1.05):
                y_ticks = np.around(np.arange(1.0, 1.055, 0.01), decimals=2)
                y_ticks1 = np.around(np.arange(1.0, 1.055, 0.005), decimals=3)
                min_v = 1.0
                max_v=1.05
            elif(max_v < 1.3):
                y_ticks = np.around(np.arange(1.0, 1.31, 0.06), decimals=2)
                y_ticks1 = np.around(np.arange(1.0, 1.31, 0.03), decimals=2)
                min_v = 1.0      
                max_v=1.30
            elif(max_v < 1.4):
                y_ticks = np.around(np.arange(1.0, 1.45, 0.08), decimals=2)
                y_ticks1 = np.around(np.arange(1.0, 1.45, 0.04), decimals=2)
                min_v = 1.0
                max_v=1.40
            elif(max_v < 1.7):
                y_ticks = np.around(np.arange(1.0, 1.71, 0.14), decimals=2)
                y_ticks1 = np.around(np.arange(1.0, 1.71, 0.07), decimals=2)
                min_v = 1.0
                max_v=1.70
            ...
            # ax.set_title(f'IPCI', fontsize=9)
            if(ax== 0 and ay == 0):
                axes[ay].legend(handles=legend_patch,loc='upper center', bbox_to_anchor=(2.45, 1.30), ncol=4, fontsize=9,
             # y 参数控制标题的位置
        handlelength=1.2,     # 控制图例句柄的长度
        handletextpad=0.2,  # 控制图例句柄和文本之间的间距
        columnspacing=0.5,
