...
#         # ax.set_xlabel('Benchmarks Suites', fontsize=8)
#         ax.set_ylabel("SpeedUp", fontsize=8)
#         # ax.set_title(f'IPCI', fontsize=8)
#         ax.legend(loc='upper center', bbox_to_anchor=(0.5, 0.83*fig_size[1]), ncol=num_prefs-1,fontsize=8,
...
if __name__ == "__main__":  
    for date in dates:
        ...
        for trace in traces:
            ...
            axes[ax, ay].set_xticklabels([''], fontsize=8)
            ...
            if(ay == 0):
                axes[ax,ay].set_ylabel("SpeedUp", fontsize=8)
            # ax.set_title(f'IPCI', fontsize=8)
            if(ax== 0 and ay == 0):
                axes[ax,ay].legend(handles=legend_patch,loc='upper center', bbox_to_anchor=(1.10, 1.30), ncol=4, fontsize=8,
             # y 参数控制标题的位置
        handlelength=1.2,     # 控制图例句柄的长度
        handletextpad=0.2,  # 控制图例句柄和文本之间的间距
        columnspacing=0.5,
        edgecolor='black'   )
            axes[ax,ay].set_title(dic_suite[trace], y=-0.28, fontsize=8) 
