...
if __name__ == "__main__":  
    for date in dates:
        ...
        ## 设置图像保存的路径
        figure_res='analysis_py/evaluation/1core_1pref_detail/'
        ...
        ax.set_xticklabels(benchs_name, fontsize=8,rotation = 45,ha ='right')
        ...
        if(max_v < 0.5):
            ...
            y_ticks = np.around(np.arange(0.8, 2.5, 0.2), decimals=1)
            y_ticks1 = np.around(np.arange(0.8, 2.5, 0.1), decimals=1)
        ax_idx = 0
        ...
        plt.savefig(f'{figure_res}/1core_1pref_IPCI_SPEC.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/1core_1pref_IPCI_SEPC.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.close()   
