...
output_path_base = "analysis_py/evaluation/sensitive_design/"
...
print(speedup[0])
########绘图，将数据加入小方格
fig, [ax1,ax2] = plt.subplots(2,1,figsize=(3.23, 3.0))
plt.subplots_adjust(left=0, right=1.0, bottom=0, top=1.0, wspace=0.4, hspace=0.45)
    ...
    if(threshs[2] == '0.70' and threshs[3] == '0.1'):
        if(L1_dic[threshs[0]] > L2_dic[threshs[1]]):
            rect = patches.Rectangle((L1_dic[threshs[0]], L2_dic[threshs[1]]), 0.1, 0.1,  # 此处假定每个方块的宽度和高度都是1
                                    linewidth=1,
                                    edgecolor='white',
                                    facecolor=plt.cm.gray_r((speedup[i]-1.555)*50))
            ax1.add_patch(rect)
...
# 设置坐标轴标签
ax1.set_xlabel('L1 threshold for page',fontsize=9)
ax1.set_ylabel('L2 threshold for page',fontsize=9)
# ax1.set_title('(a) Thresholds of Page',va='bottom')
ax2.set_xlabel('L1 threshold for pc',fontsize=9)
ax2.set_ylabel('L2 threshold for pc',fontsize=9)
...
norm = Normalize(vmin=1.555, vmax=1.575)
...
plt.show()
plt.savefig(os.path.join(output_path_base,'thresh_speedup.png'),dpi=800, format="png", bbox_inches='tight')
plt.savefig(os.path.join(output_path_base,'thresh_speedup.pdf'),dpi=800, format="pdf", bbox_inches='tight')