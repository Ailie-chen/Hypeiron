...
figure_res = 'analysis_py/evaluation/sensitive_design/'
...
fig = plt.figure(figsize=(3.23, 3.0), dpi=300)
gs = gridspec.GridSpec(2, 3, figure=fig)
ax1 = fig.add_subplot(gs[0, 0:2])
ax2 = fig.add_subplot(gs[1, 0:2])
ax3 = fig.add_subplot(gs[0, 2:3],sharey=ax1)
ax4 = fig.add_subplot(gs[1, 2:3],sharey=ax2)
...
ax1.set_xticklabels([4,8,16,32,64,96,128,256,512],fontsize=8,ha='center')
ax1.set_title('(a) Size of tables(left) and entries(right) for Page', x=0.7,y=-0.40, fontsize=9) 
...
ax2.set_xticklabels([4,8,16,32,64,96,128,256,512],fontsize=8,ha='center')
ax2.set_title('(b) Size of tables(left) and entries(right) for PC', x=0.7,y=-0.40, fontsize=9)
...
ax3.set_xticklabels([4,8,16,32,64],fontsize=8,ha='center')
# ax3.set_title('(b) Size of tables(left) and entries(right) for PC', y=-0.38, fontsize=9) 
...
# ax2.set_yticks(np.arange(150, 161,1)/100)
ax4.set_xticklabels([4,8,16,32,64],fontsize=8,ha='center')
# ax4.set_title('(d) Size of entries for PC', y=-0.38, fontsize=9)
# ax2.set_yticklabels(['{:.2f}'.format(item) for item in np.arange(150, 161,1)/100],fontsize=9,ha='right')
# ax2.set_ylabel('SpeedUp')

ax1.set_ylim(1.53, 1.58) 
ax2.set_ylim(1.53, 1.58) 
...
for item in np.arange(1500, 1601,5)/1000:
    if(ax_idx % 2 == 0):
        ...
        ax4.axhline(float(item), color='lightgray', linestyle='--', linewidth=1,zorder=1)
    ax_idx += 1
ax1.legend(loc='upper center', bbox_to_anchor=(0.5, 1.45), ncol=1, fontsize=8,
             # y 参数控制标题的位置
        handlelength=1.2,     # 控制图例句柄的长度
        handletextpad=0.2,  # 控制图例句柄和文本之间的间距
        columnspacing=0.5,
        edgecolor='black'   )
ax2.legend(loc='upper center', bbox_to_anchor=(0.5, 1.45), ncol=1, fontsize=8,
             # y 参数控制标题的位置
        handlelength=1.2,     # 控制图例句柄的长度
        handletextpad=0.2,  # 控制图例句柄和文本之间的间距
        columnspacing=0.5,
        edgecolor='black'   )
ax3.legend(loc='upper center', bbox_to_anchor=(0.5, 1.45), ncol=1, fontsize=8,
             # y 参数控制标题的位置
        handlelength=1.2,     # 控制图例句柄的长度
        handletextpad=0.2,  # 控制图例句柄和文本之间的间距
        columnspacing=0.5,
        edgecolor='black'   )
ax4.legend(loc='upper center', bbox_to_anchor=(0.5, 1.45), ncol=1, fontsize=8,
             # y 参数控制标题的位置
        handlelength=1.2,     # 控制图例句柄的长度
        handletextpad=0.2,  # 控制图例句柄和文本之间的间距
        columnspacing=0.5,
