...
markercolors=["#4472C4","#ED7D31", 'grey', "green","#4472C4","#ED7D31", 'grey', "green"]
...
for i in np.arange(0,len(match_datas)):
    ...
    if(i == 0):
        ...
        label=labels[i],
        markerfacecolor=markercolors[i], linewidth=1,markersize=5,zorder=2,color=markercolors[i]) 
    elif(i == 2):
        ax1.plot(left_bar_pos, speedup, linestyle='--', marker=markers[i],
        label=labels[i],
        markerfacecolor=markercolors[i], linewidth=1,markersize=5,zorder=2,color=markercolors[i]) 
        ...
        label=labels[i],markerfacecolor=markercolors[i], linewidth=1,markersize=5,zorder=2,color=markercolors[i]) 
    elif(i == 6) :
        ax2.plot(left_bar_pos, speedup, linestyle='--', marker=markers[i],
        label=labels[i],markerfacecolor=markercolors[i], linewidth=1,markersize=5,zorder=2,color=markercolors[i]) 
    elif( i == 1):
...
ax1.set_yticklabels(['{:.2f}'.format(item-1.0) for item in np.arange(150, 161,1)/100],fontsize=9,ha='right')
