...
def record_results(sort_item=None):
    ...
    res_name = CONFIGS["stats_dir"] + '/'+ CONFIGS["date"] + 'storage.csv'
    print(res_name)
    fig_stat_name = CONFIGS["stats_dir"]+ "_figs" + '/' + CONFIGS["date"] + 'storage.csv'
    ...
    with open(fig_stat_name, 'w') as f:
        ...
        for trace in CONFIGS["output_workloads"] +['Average']:
            ...
            if (count_dict['hyperion_hpc'] >= count_dict['mlop_dpc3']   
            and count_dict['hyperion_hpc'] >= count_dict['ipcp_isca2020']  
            and count_dict['hyperion_hpc'] >= count_dict['vberti']):
                hyperion_best += 1

            elif (count_dict['vberti'] >= count_dict['mlop_dpc3']  
            and count_dict['vberti'] >= count_dict['ipcp_isca2020']):
                berti_best += 1
                if count_dict['hyperion_hpc']>= count_dict['vberti']:
                    berti_high_higher += 1
                else:
                    berti_high_smaller += 1
            elif (count_dict['hyperion_hpc'] >= count_dict['mlop_dpc3'] 
                and count_dict['hyperion_hpc'] >= count_dict['ipcp_isca2020']):
                berti_low_higher += 1
            elif count_dict['hyperion_hpc'] < count_dict['vberti']:
                berti_low_smaller += 1
            else:
                berti_low_narrow += 1
