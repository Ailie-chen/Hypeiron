...
if __name__ == "__main__":  
    for date in dates:
        ...
        print(left_bar_pos)
        ## 设置图像保存的路径
        figure_res='analysis_py/evaluation3/thresh_L2C/'
        ...
        ax2.plot([(data-0.0)/0.76*(0.4)+1.4 for data in data_all[0:len(prefs),1]],color=colors[2],marker='o', markerfacecolor='none', linewidth=fig_line_with,markersize=2.5,zorder=2)   
        ax2.plot([(data-0.0)/0.76*(0.4)+1.4 for data in data_all[0:len(prefs),2]],color=colors[0],marker='o', markerfacecolor='none', linewidth=fig_line_with,markersize=2.5,zorder=2)   
        ...
        # 添加矩形到轴上
        ax.add_patch(patches.Rectangle((0.8, 1.45), 0.4, 0.34, linewidth=fig_line_with, edgecolor=colors[3], facecolor='none',zorder=2))
        ...
        ax.legend(handles=legend_patch,loc='upper center', bbox_to_anchor=(0.7, 0.70), ncol=1, fontsize=fontsizes,
             # y 参数控制标题的位置
        ...
        ax.set_xlabel("(d) L2C_threshold",  fontsize=fontsizes) 
        ax.text(5.2,1.75,"L2C Accuracy and",fontsize=fontsizes,ha='center',va='top',rotation = 90)
        ax.text(5.5,1.71,"L2C Coverage",fontsize=fontsizes,ha='center',va='top',rotation = 90)
        ...
        plt.savefig(f'{figure_res}/L2C.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/L2C.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.close()   
