...
DIC_MERGE: Dict[str, Dict[str, Any]] = {}
...
def record_results(sort_item=None):
    res_name = CONFIGS["stats_dir"] + '/'+ CONFIGS["date"] + 'merge.csv'
    print(res_name)
    fig_stat_name = CONFIGS["stats_dir"]+ "_figs" + '/' + CONFIGS["date"] + 'merge.csv'
    ...
    # if(sort_item=="output_prefetchers"):
    #     sort_stat[trace] = CONFIGS["output_prefetchers"]
    for trace in CONFIGS["output_workloads"]:
        sort_prefetchers = {}
        for prefetcher in CONFIGS["output_prefetchers"]:
            add_entry(DIC_MERGE,trace,prefetcher)
            print(trace)
            for field in CONFIGS["output_metrics"]:
                ...
                for trace_single, value in ROI_ORIGIN_STATS.items() :
                    if((''+trace+'_phase') in trace_single or trace == trace_single):
                ...
                DIC_MERGE[trace][prefetcher][field] = metric_value
    DIC_MERGE['Average'] = {}
    for trace in DIC_MERGE:
        if trace == 'Average':
            continue
        for prefetcher in DIC_MERGE[trace]:
            entry = DIC_MERGE[trace][prefetcher]
            avg_entry = add_entry(DIC_MERGE,'Average', prefetcher)
            for field in CONFIGS["output_metrics"]:
                if field == 'Each IPC' or "Each IPC" in field:
                    continue
                avg_entry[field].append(entry[field])
                # print(field,  prefetcher, trace, avg_entry[field])
    for prefetcher in DIC_MERGE['Average']:
        entry = DIC_MERGE['Average'][prefetcher]
        for field in CONFIGS["output_metrics"]:
            if field == 'Each IPC' or "Each IPC" in field:
                continue
            if field == 'IPCI':
        ...
        for trace in CONFIGS["output_workloads"] +['Average']:
            ...
            for field in CONFIGS["output_metrics"]:
                for prefetcher in sort_stat[trace] :
                    ...
                    # else:
                    #     line.append(metric_value)
                    metric_value = DIC_MERGE[trace][prefetcher][field]
                    line.append(metric_value)
    ...
    with open(res_name, 'w') as f:
        ...
        for trace in ['Average']:
            ...
            for prefetcher in sort_stat[trace] :
                if(trace not in DIC_MERGE or prefetcher not in DIC_MERGE[trace] or prefetcher not in CONFIGS["output_prefetchers"]):
                    print(trace, prefetcher)
                    print("exist: ",(trace not in DIC_MERGE), (prefetcher not in DIC_MERGE[trace]), (prefetcher not in CONFIGS["output_prefetchers"]))
                    assert(0)
