...
if __name__ == "__main__":  
    for date in dates:
        ...
        for i in range(num_prefs):
            ...
            y_ticks=np.around(np.arange(y_min, y_max, 0.1), decimals=1)
            y_ticks1=np.around(np.arange(y_min, y_max+0.01, 0.2), decimals=1)
            ...
            ax.legend(handles=legend_patch,loc='upper center', bbox_to_anchor=(0.5, 1.30), ncol=4, fontsize=fontsizes,
             # y 参数控制标题的位置
        ...
        plt.savefig(f'{figure_res}/L1D_Coverage.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/L1D_Coverage.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.close()   
