...
metrics=['L1D MPKI','L2C MPKI', "LLC MPKI"]
...
if __name__ == "__main__":  
    for date in dates:
        ...
        for trace in traces:
            ...
            axes[ax, ay].set_xticklabels(['L1D','L2C','LLC'], fontsize=8)
            # axes[ax,ay].set_xlabel('Benchmarks', fontsize=8)
            axes[ax, ay].set_ylim(min_v, max_v)

            if(max_v < 5):
                y_ticks = np.around(np.arange(0, 6, 1), decimals=0)
                y_ticks1 = np.around(np.arange(0, 5.5, 0.5), decimals=1)
            elif(max_v < 10):
                y_ticks = np.around(np.arange(0, 11, 2), decimals=0)
                y_ticks1 = np.around(np.arange(0, 11, 1), decimals=0)
            elif(max_v < 20):
                y_ticks = np.around(np.arange(0, 21, 4), decimals=0)
                y_ticks1 = np.around(np.arange(0, 21, 2), decimals=0)
            elif(max_v < 50):
                y_ticks = np.around(np.arange(0, 60, 10), decimals=0)
                y_ticks1 = np.around(np.arange(0, 60, 5), decimals=0)
            elif(max_v < 100):
            ...
            if(ax== 0 and ay == 0):
                axes[ax,ay].legend(loc='upper center', bbox_to_anchor=(1.2, 1.30), ncol=num_prefs-1, fontsize=8,
             # y 参数控制标题的位置
        ...
        plt.savefig(f'{figure_res}/1core_1pref_MPKI.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/1core_1pref_MPKI.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.close()   
