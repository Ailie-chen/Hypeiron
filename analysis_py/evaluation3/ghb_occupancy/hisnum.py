...
if __name__ == "__main__":  
    for date in dates:
        ...
        print(left_bar_pos)
        ## 设置图像保存的路径
        figure_res='analysis_py/evaluation3/ghb_occupancy/'
        ...
        # 添加矩形到轴上
        ax.add_patch(patches.Rectangle((1.85, 1.45), 0.3, 0.1, linewidth=fig_line_with, edgecolor=colors[3], facecolor='none',zorder=2))

        # 计算x轴刻度的位置
        x_ticks = left_bar_pos
        ax.set_xticks(x_ticks)
        ax.set_xticklabels([4,6,8,12,16,32,64], fontsize=fontsizes)
        ...
        ax.text(-1.72,1.4,"SpeedUp",fontsize=fontsizes,ha='center',va='top',rotation = 90)
        ...
        ax.text(0.5,0.85,r'(a) Max Number of histories for',fontsize=fontsizes-1)
        ax.text(0.8,0.80,r' each PC and page',fontsize=fontsizes-1)




        plt.savefig(f'{figure_res}/hisnum.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/hisnum.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.close()   
