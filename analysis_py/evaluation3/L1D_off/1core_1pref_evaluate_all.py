...
def parse_file(file: str) -> Tuple[str, str, int, Dict[str, Any]]:
    ...
    with open(file, mode='r', errors='ignore') as f:
        ...
        for line in f.readlines():
            ...
            pattern_page_occupancy = '^Average ghb_page_occupancy: (\d+\.\d+)'
            pattern_pc_occupancy = '^Average ghb_pc_occupancy: (\d+\.\d+)'
            matches_page_occupancy = re.search(pattern_page_occupancy, line)
            matches_pc_occupancy = re.search(pattern_pc_occupancy, line)
            if matches_page_occupancy:
                entry["GHB PAGE OCCUPANCY"][current_cpu] = float(matches_page_occupancy.group(1))
            if matches_pc_occupancy:
                entry["GHB PC OCCUPANCY"][current_cpu] = float(matches_pc_occupancy.group(1))
