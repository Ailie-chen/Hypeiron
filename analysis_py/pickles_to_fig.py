...
for i, value in enumerate(rdip):
    ax1.scatter(i + 0.5, value+0.2, marker='D',edgecolors='black', facecolors='white',s=8,zorder=3,linewidth=1)  # i + 0.5 将圆圈右移0.5个单位 #,label='distribution of $RD_{PC}' if i==0 else ""
scatter_legend = plt.Line2D([0], [0], marker='D', color='w', markerfacecolor='white', markeredgecolor='black', label='distribution of $RD_{PC}$')

ax1.legend(handles=[legend_rect, scatter_legend], loc='upper center', bbox_to_anchor=(0.55, 1.22), ncol=1,fontsize=8)
